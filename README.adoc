...
`MyList<T>`
* {GENERATE} generates new classes by performing string or
  regex replacements on the source code

=== Why should I use it?
The single biggest advantage of this library over generic string template
engines such as
...
template file, your "template" is a Java class, which has the following
benefits:
...
== Generate classes using {GENERATE}
